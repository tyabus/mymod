...
void CSqueakGrenade :: Spawn( void )
{
	...
	m_flDie = gpGlobals->time + RANDOM_LONG( 10, 20 );
...
void CSqueakGrenade::HuntThink( void )
{
	...
	float flpitch = 155.0 - 60.0 * ( (m_flDie - gpGlobals->time) / RANDOM_LONG( 10, 20 ) );
	if (flpitch < 80)
		flpitch = 80;
...
void CSqueakGrenade::SuperBounceTouch( CBaseEntity *pOther )
{
	...
	// higher pitch as squeeker gets closer to detonation time
	flpitch = 155.0 - 60.0 * ((m_flDie - gpGlobals->time) / RANDOM_LONG( 10, 20 ) );
