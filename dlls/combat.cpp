...
void CGib :: SpawnStickyGibs( entvars_t *pevVictim, Vector vecOrigin, int cGibs )
{
	...
	for ( i = 0 ; i < cGibs ; i++ )
	{
		...
		if ( pevVictim )
		{
			pGib->pev->origin.x = vecOrigin.x + RANDOM_FLOAT( -5, 4 );
			pGib->pev->origin.y = vecOrigin.y + RANDOM_FLOAT( -5, 4 );
			pGib->pev->origin.z = vecOrigin.z + RANDOM_FLOAT( -5, 4 );

			// make the gib fly away from the attack vector
			pGib->pev->velocity = g_vecAttackDir * -1;

			// mix in some noise
			pGib->pev->velocity.x += RANDOM_FLOAT( -0.20, 0.20 );
			pGib->pev->velocity.y += RANDOM_FLOAT( -0.20, 0.20 );
			pGib->pev->velocity.z += RANDOM_FLOAT( -0.20, 0.20 );

			pGib->pev->velocity = pGib->pev->velocity * RANDOM_LONG( 700, 850 );

			pGib->pev->avelocity.x = RANDOM_FLOAT( 250, 450 );
			pGib->pev->avelocity.y = RANDOM_FLOAT( 250, 450 );
...
void CGib :: SpawnHeadGib( entvars_t *pevVictim )
{
	...
	if ( pevVictim )
	{
		...
		{
			pGib->pev->velocity = Vector ( RANDOM_FLOAT( -150, 150 ), RANDOM_FLOAT( -150, 150 ), RANDOM_FLOAT( 200, 350 ) );
		}


		pGib->pev->avelocity.x = RANDOM_FLOAT ( 100, 250 );
		pGib->pev->avelocity.y = RANDOM_FLOAT ( 100, 350 );
...
void CBaseMonster :: GibMonster( void )
{
	...
	if ( HasHumanGibs() )
	{
		if ( CVAR_GET_FLOAT("violence_hgibs") != 0 )	// Only the player will ever get here
		{
			...
			CGib::SpawnRandomGibs( pev, RANDOM_FLOAT( 6, 8 ), 1 );	// throw some human gibs.
		}
		gibbed = TRUE;
	}
	else if ( HasAlienGibs() )
	{
		...
		{
			CGib::SpawnRandomGibs( pev, RANDOM_FLOAT( 6, 8 ), 0 );	// Throw alien gibs
		}
		gibbed = TRUE;
	}
