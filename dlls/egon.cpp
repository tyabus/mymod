...
void CEgon::Fire( const Vector &vecOrigSrc, const Vector &vecDir )
{
	...
	case FIRE_WIDE:
		...
		if ( pev->dmgtime < gpGlobals->time )
		{
			...
			if (pEntity->pev->takedamage)
			{
				pEntity->TraceAttack( m_pPlayer->pev, gSkillData.plrDmgEgonWide/1.5, vecDir, &tr, DMG_ENERGYBEAM | DMG_ALWAYSGIB);
			}
			ApplyMultiDamage(m_pPlayer->pev, m_pPlayer->pev);
			...
			if ( g_pGameRules->IsMultiplayer() )
			{
				...
				::RadiusDamage( tr.vecEndPos, pev, m_pPlayer->pev, gSkillData.plrDmgEgonWide/RANDOM_FLOAT( 4, 6 ), 128, CLASS_NONE, DMG_ENERGYBEAM | DMG_BLAST | DMG_ALWAYSGIB );
			}
			...
			{
				UTIL_ScreenShake( tr.vecEndPos, 5.0, 130.0, 0.70, 200.0 );
				m_shakeTime = gpGlobals->time + 2.0;
			}
		}
#endif
