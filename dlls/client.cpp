...
void Host_Say( edict_t *pEntity )
{
	...
	if ( !player->pev->netname && STRING(player->pev->netname)[0] == 0 )
		return;		// Player dosen't have nickname and want to talk?
...
// turn on color set 2  (color on,  no sound)
	...
	while ( ((client = (CBasePlayer*)UTIL_FindEntityByClassname( client, "player" )) != NULL) && (!FNullEnt(client->edict())) ) 
	{
		...
		if ( !player->IsObserver() && g_pGameRules->PlayerRelationship(client, CBaseEntity::Instance(pEntity)) != GR_TEAMMATE )
			continue;
