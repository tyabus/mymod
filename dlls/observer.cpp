...
void CBasePlayer::Observer_FindNextPlayer( bool bReverse )
{
	...
	do
	{
		...
		if( ( (CBasePlayer*)pEnt )->IsObserver() )
			m_hObserverTarget = NULL;
			continue;
		if( pEnt->pev->takedamage == DAMAGE_NO && pEnt->pev->solid == SOLID_NOT )
			m_hObserverTarget = NULL;
			continue;
...
void CBasePlayer::Observer_CheckTarget()
{
	...
	if( target->pev->takedamage == DAMAGE_NO && target->pev->solid == SOLID_NOT )
        {
                Observer_FindNextPlayer( false );
                return;
        }
...
void CBasePlayer::Observer_SetMode( int iMode )
{
	...
	if( m_hObserverTarget != 0 )
	{
		...
		if( ( pEnt == this ) || ( pEnt == NULL ) )
			m_hObserverTarget = 0;
		else if( ( (CBasePlayer*)pEnt )->IsObserver() || ( pEnt->pev->takedamage == DAMAGE_NO && pEnt->pev->solid == SOLID_NOT ) )
			m_hObserverTarget = 0;
	}
